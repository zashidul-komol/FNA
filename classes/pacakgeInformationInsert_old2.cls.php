			 ...
			 if ($_FILES[csv][size] > 0) { 
				...
				while (($data = fgetcsv($handle, 1000, ",")) !== FALSE)
				{
					...
					if($chk <= 6){
						...
						if($data[0] == 'Total value of Goods Procurement'){
							$asfasdf = 'End Operation.';
						}else{
						...
						$pi_17_csv = addslashes($data[5]);  $pi_7c_csv = addslashes($data[6]);  $pi_7d_csv = addslashes($data[7]);
						if($piProcurementType == 2){
							$used_for_works = insertDateMySQlFormat($data[8]); 
						}
						$bps_38_csv = insertDateMySQlFormat($data[9]); 
						
						$bpes_50_csv = insertDateMySQlFormat($data[10]); 
						$bpes_51_csv = insertDateMySQlFormat($data[11]); $eras_63_csv = insertDateMySQlFormat($data[12]);  
						$cs_64_csv = insertDateMySQlFormat($data[13]);  
						$cs_68_csv = insertDateMySQlFormat($data[14]); $cms_74_csv = insertDateMySQlFormat($data[16]);  
						$test = insertDateMySQlFormat($data[17]);  
						
						$s = addslashes($data[18]);
						$t = addslashes($data[19]);  $u = addslashes($data[20]);  $v = addslashes($data[21]);
						$w = addslashes($data[22]);  $x = addslashes($data[23]); 
