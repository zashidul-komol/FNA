<?php
	...
	if(isset($opId) && (strlen($opId) > 0)) {
		...
		if(isset($_POST['InsertPartyPaymentReceiveOpening'])) {
			$PalotInsert 	= new projectSetupInsertNew();
			$msg 				= $PalotInsert->InsertPartyPaymentReceiveOpening($userId);
		}
		// Insert Palot Information End
		
		$body 				= $action->getPartyPaymentReceiveEntryOpening($userId);
		if(strlen($msg)>0) {
