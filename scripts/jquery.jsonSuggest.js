...
(function($){
	$.fn.jsonSuggest = function(searchData,settings) {
		var defaults = {minCharacters:1,maxResults:undefined,wildCard:"",caseSensitive:false,notCharacter:"!",maxHeight:250,highlightMatches:true,onSelect:undefined,ajaxResults:false,defaultId:1};
		...
		return this.each(function(){
			...
			function selectResultItem(item) {
				...
				if(typeof settings.onSelect === 'function') {
					settings.onSelect(item,settings.defaultId);
				}
			}
			...
			function buildResults(resultObjects,sFilterTxt) {
				...
				for(i = 0; i < resultObjects.length; i += 1) {
					var item = $('<div />'), text = resultObjects[i].text, id = resultObjects[i].id;
					
					if((id.substr(-6,3) == '000') && (id.substr(-3,3) == '000')) {
						$(item).append('<p class="text" style="font-weight:bold;">'+text+'</p>');
						
						$(item).addClass('disabledItems').addClass((bOddRow)?'odd':'even');
					...
					} else if((id.substr(-6,3) != '000') && (id.substr(-3,3) == '000')) {
						$(item).append('<p class="text" style="padding-left:30px">'+text+'</p>');
						...
						$(item).append('<p class="text" style="padding-left:60px">'+text+'</p>');
			...
			function runSuggest(e) {
				...
				sFilterTxt=sFilterTxt||'.*';sFilterTxt=settings.wildCard?'^'+sFilterTxt:sFilterTxt;filterPatt=settings.caseSensitive?new RegExp(sFilterTxt):new RegExp(sFilterTxt,"i");
				if(settings.ajaxResults===true) {
					...
					for(i=0;i<searchData.length;i+=1) {
						if((filterPatt.test(searchData[i].text) === bMatch) || (searchData[i].id.substr(-3,3) == '000')) {
							resultObjects.push(searchData[i]);
						}
					}
				}
				//alert(JSON.stringify(resultObjects));
				...
				switch(e.keyCode){
					...
					case 40:
						if(typeof currentSelection==='undefined') {
							...
							currentSelection=$(currentSelection).nextAll('.resultItem').get(0);
						}
					...
					case 38:
						if(typeof currentSelection==='undefined') {
							...
							currentSelection=$(currentSelection).prevAll('.resultItem').get(0);
						}
						setHoverClass(currentSelection);
						if(currentSelection) {
							$(results).scrollTop(currentSelection.offsetTop);
						...
						return false;
					default:runSuggest.apply(this,[e]);
				}
			}
			
			$(results).addClass('jsonSuggestResults').css({'top':(obj.position().top+obj.height()+5)+'px','left':obj.position().left+'px','width':(obj.width() <= 0 ) ? 450 : obj.width() +'px'}).hide();
