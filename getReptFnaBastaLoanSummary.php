								...
								*/
								...
								foreach($LOTNO_ARRAY_UNIQUE as $individualLotNo){
									...
									$BastaLoanViewQuery 	= "SELECT 
										...
										/*
										echo  "SELECT sum(SELLQNTY) as SELLQNTY,
																sum(SELLPRICE) as SELLPRICE,
																sum(RECEIVEDAMOUNT) as RECEIVEDAMOUNT
														FROM 
															fna_basta 
														WHERE ENTRYDATE BETWEEN '2022-01-01' AND '".$ENTRYDATE_TO."'
														{$con}
														AND LOTNO = '".$individualLotNo."'"; */
														
										$BastaViewQuery 	= "SELECT sum(SELLQNTY) as SELLQNTY,
																sum(SELLPRICE) as SELLPRICE,
																sum(RECEIVEDAMOUNT) as RECEIVEDAMOUNT
														FROM 
															fna_basta 
														...
														AND LOTNO = '".$individualLotNo."'
														";
										$BastaViewQueryStatement				= mysql_query($BastaViewQuery);
										
										while($BastaViewQueryStatementData	= mysql_fetch_array($BastaViewQueryStatement)){ 
										
										 $SELLQNTY_NEW     							= $BastaViewQueryStatementData["SELLQNTY"];
										 $SELLPRICE_NEW    							= $BastaViewQueryStatementData["SELLPRICE"];
										 $RECEIVEDAMOUNT_NEW						= $BastaViewQueryStatementData["RECEIVEDAMOUNT"];
										 
										}
										
										$Total_SELLQNTY = $Total_SELLQNTY + $SELLQNTY_NEW ; 
										$Total_SELLPRICE = $Total_SELLPRICE + $SELLPRICE_NEW ; 
										$Total_RECEIVEDAMOUNT = $Total_RECEIVEDAMOUNT + $RECEIVEDAMOUNT_NEW ; 
										
										$BalanceAmount		= $SELLPRICE_NEW - $RECEIVEDAMOUNT_NEW ; 
									...
									$tableView .=" <tr>
														...
														<td align='center' valign='top' width='10%' style='border: 1px dotted #000'>$individualLotNo</td>
														
														<td align='center' valign='top' width='10%' style='border: 1px dotted #000'>$SELLQNTY_NEW</td>
														
														<td align='center' valign='top' width='10%' style='border: 1px dotted #000'>$UNITPRICE</td>
								
														<td align='right' valign='top' width='10%' style='border: 1px dotted #000'>$SELLPRICE_NEW</td>
														
														<td align='right' valign='top' width='10%' style='border: 1px dotted #000'>$RECEIVEDAMOUNT_NEW</td>
