...
// Query here.
						...
						while($BasicQueryStatementData	= mysql_fetch_array($BasicQueryStatement)){	
							...
							while($partyPaymentAmountStatementData			= mysql_fetch_array($partyPaymentAmountStatement)) {
									$BILLAMOUNT 							= $partyPaymentAmountStatementData["billamnt"];	
								}
							
								$globalRecAmount 	= $globalRecAmount + $RECEIVEAMOUNT ; 
								$partyBalAmount 	= $Global_Total_Price - $BILLAMOUNT ; 
				...
				$FeedSellQuery 	= "SELECT fs.FOODID,
										  fs.QUANTITY,
										  fs.AMOUNT,
										  fs.SELLAVGPRICE,
										  fs.FOODFLAG,
										...
										FROM feed_finishedstock fs, feed_fooditem fi
										WHERE fi.FOODID = fs.FOODID
										AND fs.ENTDATE BETWEEN '".$OPENINGDATE_FROM."' AND '".$OPENINGDATE_TO."'
										AND fs.PROJECTID = '3'
										AND fs.SUBPROJECTID = '8'
				...
				while($FeedSellQueryStatementData	= mysql_fetch_array($FeedSellQueryStatement)){	
					...
					$SELLAVGPRICE	 			= $FeedSellQueryStatementData['SELLAVGPRICE'];
				...
				$tableView .="
							<tr>
								...
								<td align='center' valign='top'  style='border: 1px dotted #000'>Hatchery</td>
							...
							<td colspan='8' align='left' valign='top'>
								<table width='50%' border='0' cellpadding='3' cellspacing='0' style='margin:0px auto 0px auto;font-size:100%;'>
								  ...
								  <tr>
									...
									<td width='2%' style='border: 1px dotted #000'>:</td>
									<td width='48%' style='border: 1px dotted #000'>".number_format($RECEIVEAMOUNT,2)."</td>
								  </tr>
								  <tr>
									<td align='right' width='50%' style='border: 1px dotted #000'>Purchase Balance</td>
									<td width='2%' style='border: 1px dotted #000'>:</td>
									<td width='48%' style='border: 1px dotted #000'>".number_format($BILLAMOUNT,2)."</td>
								  </tr>
								  <tr>
