...
function insertDateMySQlFormat($date){
	...
	$userId 			= $_REQUEST['userId'];
		...
		$FloorSql 	= "
							...
							AND FLOORID = $FLOORID
						";
			$FloorSqlStatement				= mysql_query($FloorSql);
			while($FloorSqlStatementData	= mysql_fetch_array($FloorSqlStatement)){
				$FLOORNAME	        		= $FloorSqlStatementData["FLOORNAME"];
			}
	...
	$tableView .="<table width='80%' border='0' cellpadding='3' cellspacing='0' style='margin:0px auto 0px auto;font-size:85%; border: 1px dotted #000'>
					  ...
					  <tr>
						<td colspan='13' align='center' valign='middle' style='border: 1px dotted #000'>
							...
							<center>&nbsp;</center>
							<center><b><font size=4>Chamber - $CHNAME - Floor: $FLOORNAME</FONT></b></center>
							<center>&nbsp;</center>
...
// Query here.
							...
							// Package Information View Report Start 	 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<		
								...
								while($aViewStatementData	= mysql_fetch_array($aViewStatement)){ 
								 ...
								 $FLOORID_NEW	        = $aViewStatementData["FLOORID"];
						...
						$tableView .=" <tr>
											...
											<td align='center' valign='top' style='border: 1px dotted #000' width='18%'> $PARTYNAME</td>
											
											<td align='right' valign='top' style='border: 1px dotted #000' width='15%'> $PRODUCTNAME</td>
											
											<td align='right' valign='top' style='border: 1px dotted #000' width='12%'> $NewPackingName </td>
