...
// Query here.
						...
						while($BasicQueryAStatementData		= mysql_fetch_array($BasicQueryAStatement)){	
							$PRODUCTID_ARRAY[] 				= $BasicQueryAStatementData['PRODUCTID'];
							$i++;
						}
						
						$PRODUCTID_ARRAY_UNIQUE = array_unique($PRODUCTID_ARRAY);
						$SL 					=1;
						$Global_Medicine_Qnty	=0;	
						$Global_Total_Taka		=0;	
						foreach($PRODUCTID_ARRAY_UNIQUE as $individualProduct){
						
						$BatchOpenQuery 		= "SELECT 	SUM(QUANTITY) QUANTITY,
															SUM(TOTALPRICE) TOTALPRICE,
															BATCHNO
														FROM pal_medicine
														WHERE MDDATE BETWEEN '".$OPENINGDATE_FROM."' AND '".$OPENINGDATE_TO."'
														AND PRODUCTID = '".$individualProduct."'
														AND BATCHNO = '".$BATCHNO."'
														ORDER BY PRODUCTID ASC
												";
						...
						while($BatchOpenQueryStatementData		= mysql_fetch_array($BatchOpenQueryStatement)){	
							$QUANTITY		 					= $BatchOpenQueryStatementData['QUANTITY'];
							$TOTALPRICE					 		= $BatchOpenQueryStatementData['TOTALPRICE'];
							$BATCHNO_Medicine			 		= $BatchOpenQueryStatementData['BATCHNO'];
						}
														
						$DailyOperation_Query 		= "SELECT 	PRODUCTNAME
															FROM fna_product
							...
							while($DailyOperation_QueryStatementData	= mysql_fetch_array($DailyOperation_QueryStatement)){	
								$PRODUCTNAME		 		 			= $DailyOperation_QueryStatementData['PRODUCTNAME'];
								
							}
							
							if($QUANTITY != NULL){
								
							$Global_Medicine_Qnty		= $Global_Medicine_Qnty + $QUANTITY ;
							$Global_Total_Taka			= $Global_Total_Taka + $TOTALPRICE ; 
							$Avg_Price					= $TOTALPRICE / $QUANTITY ; 		
							$tableView .=" <tr>
												<td align='center' valign='top' style='border: 1px dotted #000'>$SL</td>
	
												<td align='center' valign='top'  style='border: 1px dotted #000'>$BATCHNO_Medicine</td>
					...
					$tableView .="
						...
						<tr style='font-weight:bold;' bgcolor='#CCCCCC'>
							...
							<td align='right' valign='top' style='border: 1px dotted #000'>".number_format($Global_Medicine_Qnty,2)."</td>
