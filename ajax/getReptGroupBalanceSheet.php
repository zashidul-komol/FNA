...
function insertDateMySQlFormat($date){
	...
	$tableView = "";	
	$tableView .="<table width='65%' border='0' cellpadding='3' cellspacing='0' style='margin:0px auto 0px auto;font-size:100%; border: 1px dotted #000'>
...
// Query here.
						...
						$ProjectQuery 	= "SELECT PROJECTID
												FROM fna_project 
												ORDER BY PROJECTID , PROJECTNAME ASC
											";
						...
						if($individualProject == 2){
							...
							$Global_Balance_Income	= $Global_Balance_Income + $RECEIVEAMOUNT_party_Income ; 
							$Global_Balance_Expanse	= $Global_Balance_Expanse + $Expanse_Amount ; 
							$Global_Balance_Payable = $Global_Balance_Payable + $Total_Amount_Payable_Feed ; 
							$Global_Balance_Receiable	= $Global_Balance_Receiable + $Receivable_Amount ; 
							...
							$Loan_Query 	= "SELECT 	  sum(l.LOANAMOUNT) LOANAMNT,
												...
												AND l.LOANDATE BETWEEN '".$ENTRYDATE_FROM."' AND '".$ENTRYDATE_TO."' 
							...
							$Loan_Payment_Query 	= "SELECT 	  sum(l.LOANPAYMENT) LOANPAYMENTAMNT
														FROM fna_loan l
														WHERE l.PROJECTID = '".$individualProject."'
														AND l.LOAN_PAYMENTDATE BETWEEN '".$ENTRYDATE_FROM."' AND '".$ENTRYDATE_TO."' 
														
													";
							$Loan_Payment_QueryStatement				= mysql_query($Loan_Payment_Query);
							while($Loan_Payment_QueryStatementData		= mysql_fetch_array($Loan_Payment_QueryStatement)){	
								$LOANPAYMENT_ALU						= $Loan_Payment_QueryStatementData['LOANPAYMENTAMNT'];
							...
							$Income_Bill_Query_Cold 	= "SELECT sum(inc.AMOUNT) amount
																FROM fna_income inc 
																WHERE inc.PROJECTID = '".$individualProject."'
																AND inc.STATUS = 'Active'
																AND inc.INDATE BETWEEN '".$ENTRYDATE_FROM."' AND '".$ENTRYDATE_TO."'
																ORDER BY inc.INDATE ASC
															"; 
							$Income_Bill_Query_ColdStatement				= mysql_query($Income_Bill_Query_Cold);
							while($Income_Bill_Query_ColdStatementData		= mysql_fetch_array($Income_Bill_Query_ColdStatement)){	
								$Others_Income_Cold	 						= $Income_Bill_Query_ColdStatementData['amount'];
								
							}
							
							$FNA_Bill_Query 	= "SELECT 	sum(fb.BILLAMOUNT) BILLAMOUNT,
															sum(fb.SERVICES_CHARGE) SERVICECHARGE,
															sum(fb.OTHERS_INCOME) OTHERSINCOME	
													FROM fna_bill fb
							...
							while($FNA_Bill_QueryStatementData	= mysql_fetch_array($FNA_Bill_QueryStatement)){	
								...
								$SERVICECHARGE_FNA	 			= $FNA_Bill_QueryStatementData['SERVICECHARGE'];
								$OTHERSINCOME_FNA	 			= $FNA_Bill_QueryStatementData['OTHERSINCOME'];
							...
							$Booking_Query 	= "SELECT 	sum(book.BOOKINGMONEY) BOOKINGMONEY,
													...
													WHERE book.PROJECTID = '".$individualProject."'
							...
							$Commission_Query 	= "SELECT 	sum(comm.TOTALCOMMISSION) TOTALCOMMISSION
													...
													WHERE comm.PROJECTID = '".$individualProject."'
							...
							$Income_Amount_Cold		= $RECEIVEAMOUNT_COLD + $Others_Income_Cold + $LOANPAYMENT_ALU + $BOOKINGMONEY_ALU + $SERVICECHARGE_FNA + $OTHERSINCOME_FNA; 
							$Expanse_Amount_Cold	= $PAYMENTAMOUNT_COLD + $Expanse_Head_Cold	+ $PAYMENTAMOUNT_LAB_BILL + $LOANAMOUNT_ALU + $ADJUSTMENTMONEY_ALU + $TOTALCOMMISSION_ALU; 
						...
						while($OthersIncome_QueryStatementData		= mysql_fetch_array($OthersIncome_QueryStatement)){	
							...
							while($VangaEgg_QueryStatementData		= mysql_fetch_array($VangaEgg_QueryStatement)){	
								echo $price_vangaegg 					= $VangaEgg_QueryStatementData['price']; 
							...
							*/
							...
							$Poultry_Total_Income			= $totprice_eggsell + $totalprice_mmsell + $sellprice_doper + $INCOMEAMOUNT_othersIncome + $AMOUNT_Income_Poul;
							$Poultry_Total_Income_Old		= $RECEIVEAMOUNT_POUL + $INCOMEAMOUNT_othersIncome + $AMOUNT_Income_Poul;
						
							$Global_Others_Income			= $Global_Others_Income + $INCOMEAMOUNT_othersIncome  + $AMOUNT_Income_Poul ; 
							...
							$Expanse_Poultry				= $totprice_medicineDist + $totprice_foodDist + $batch_price + $Expanse_Head_Poul + $EXPANSEAMOUNT_othersExpanse; 
							$Expanse_Poultry_Old			= $PAYMENTAMOUNT_POUL + $Expanse_Head_Poul + $EXPANSEAMOUNT_othersExpanse; 
							
							$Total_Amount_income_Poul		= $totprice_eggsell + $totalprice_mmsell +  $sellprice_doper + $price_vangaegg + $INCOMEAMOUNT_othersIncome + $AMOUNT_Income_Poul ;
							...
							$Balance_Poul					= $Total_Amount_income_Poul - $Expanse_Poultry ;
							$Global_Balance_Income			= $Global_Balance_Income + $Total_Amount_income_Poul;
							...
							$Global_Balance_Payable 		= $Global_Balance_Payable  ;
							...
							$tableView .=" <tr>
											...
											<td align='right' valign='top' style='border: 1px dotted #000'>".number_format($Global_Rec_Amount_Pal,2)."</td>
							...
							$PartyBill_QueryStatement			= mysql_query($PartyBill_Query);
							while($PartyBill_QueryStatementData	= mysql_fetch_array($PartyBill_QueryStatement)){	
								$RECEIVEAMOUNT_Hatch			= $PartyBill_QueryStatementData['RECEIVEAMOUNT']; 
							...
							$Income_Bill_QueryStatement				= mysql_query($Income_Bill_Query);
							while($Income_Bill_QueryStatementData	= mysql_fetch_array($Income_Bill_QueryStatement)){	
								$AMOUNT_Income_Hatch 				= $Income_Bill_QueryStatementData['amount'];
								
							}
							$Global_Others_Income_Hatch		 = $Global_Others_Income_Hatch + $AMOUNT_Income_Hatch ;
							...
							$Total_Amount_income_Hatch			= $RECEIVEAMOUNT_Hatch;
							//$Total_Amount_income_global			= $Total_Amount_income_global + $Total_Amount_income_Hatch ;
							
							$Total_Amount_Expanse_Hatch		 	= $PAYMENTAMOUNT_Hatch + $AMOUNT_exp_Hatch ; 
							...
							$Balance_Hatch						= $Total_Amount_income_Hatch - $Total_Amount_Expanse_Hatch ;
							$Total_Amount_Payable_Hatch			= abs($OpenHatchEgg_price - $PAYMENTAMOUNT_Hatch)  ;
							
							$Global_Balance						= $Global_Balance + $Balance_Hatch ;
							$Global_Balance_Income				= $Global_Balance_Income + $Total_Amount_income_Hatch ;
							...
							$tableView .=" <tr>
											...
											<td align='right' valign='top' style='border: 1px dotted #000'>".number_format($Total_Amount_income_Hatch,2)."</td>
									 ...
									 ";
										...
										$Income_Bill_Query_ColdStatement				= mysql_query($Income_Bill_Query_Cold);
										while($Income_Bill_Query_ColdStatementData		= mysql_fetch_array($Income_Bill_Query_ColdStatement)){	
											$Others_Income		 						= $Income_Bill_Query_ColdStatementData['amount'];
										...
										$Income_Amount_Medicine			= $RECEIVEAMOUNT_party_Medicine + $Others_Income ; 	
										$Balance_Amount_Medicine		= $Income_Amount_Medicine - $PAYMENTAMOUNT_party_Medicine ; 
										...
										$Global_Balance_Income			= $Global_Balance_Income + $Income_Amount_Medicine ;
										$Global_Balance_Expanse			= $Global_Balance_Expanse + $PAYMENTAMOUNT_party_Medicine ; 
										$Global_Balance_Payable 		= $Global_Balance_Payable + $Medicine_Expanse_Payable ; 
										$Global_Balance_Receiable		= $Global_Balance_Receiable + $Medicine_Expanse_Receivable ; 
							
										
										$tableView .=" <tr>
														<td align='center' valign='top' style='border: 1px dotted #000'>$PROJECTNAME</td>
			
														<td align='right' valign='top' style='border: 1px dotted #000'>".number_format($Income_Amount_Medicine,2)."</td>
							...
							$RMP_Query_DanKhat 	= "SELECT sum(rmp.AMOUNT) amount
										...
										$Balance_Amount_DanKhat			= $RECEIVEAMOUNT_party_DanKhat - $PAYMENTAMOUNT_party_DanKhat ; 
										$DanKhat_Expanse_Receivable		= $Sell_BillAmount_DanKhat - $RECEIVEAMOUNT_party_DanKhat ;
										$DanKhat_Expanse_Payable		= $Pur_BillAmount_DanKhat - $PAYMENTAMOUNT_party_DanKhat ;
							...
							//------------------------------------------------------------------------
							$PartyStatementQuery_ComDev 	= "SELECT p.PARTYID
																	FROM fna_party p 
																	WHERE p.PROJECTID = '".$individualProject."'
																	ORDER BY p.PARTYID ASC
																"; 
								$PartyStatementQuery_ComDevStatement				= mysql_query($PartyStatementQuery_ComDev);
								$i = 0;
								while($PartyStatementQuery_ComDevStatementData		= mysql_fetch_array($PartyStatementQuery_ComDevStatement)){	
									$PARTYID_ARRAY_COMDEV[]							= $PartyStatementQuery_ComDevStatementData['PARTYID'];
									$i++;
								}
								
								$PARTYID_ARRAY_UNIQUE_COMDEV = array_unique($PARTYID_ARRAY_COMDEV);
								$sl = 1;	
								$Global_PurchaseBillAmount	= 0;
								$Global_SellBillAmount		= 0;
								$Global_PaymentAmount		= 0;
								$Global_receiveAmount		= 0;
								$Global_Rec_Amount_ComDev = 0;
								$Global_Pay_Amount_ComDev = 0;
									foreach($PARTYID_ARRAY_UNIQUE_COMDEV as $individualPartyComDev){
									
									$Party_Query_ComDev 	= "SELECT 	sum(p.PUR_BILLAMOUNT) PURBILLAMOUNT,
																		sum(p.SELL_BILLAMOUNT) SELLBILLAMOUNT,
																		sum(p.PAYMENTAMOUNT) PAYMENTAMOUNT,
																		sum(p.RECEIVEAMOUNT) RECEIVEAMOUNT,
																		party.PARTYNAME
																			...
																			WHERE party.PARTYID = p.PARTYID
																			AND p.PROJECTID = '".$individualProject."'
																			AND p.PARTYID = '".$individualPartyComDev."'
																			AND p.ENTRYDATE BETWEEN '".$ENTRYDATE_FROM."' AND '".$ENTRYDATE_TO."'
																			
																		";
								$Party_Query_ComDevStatement						= mysql_query($Party_Query_ComDev);
								$Rec_Amount_ComDev = 0;
								$Pay_Amount_ComDev	 = 0;
								$Balance_Amount_ComDev = 0;
								while($Party_Query_ComDevStatementData				= mysql_fetch_array($Party_Query_ComDevStatement)){	
									$PURBILLAMOUNT_COMDEV							= $Party_Query_ComDevStatementData['PURBILLAMOUNT']; 
									$SELLBILLAMOUNT_COMDEV							= $Party_Query_ComDevStatementData['SELLBILLAMOUNT'];
									$PAYMENTAMOUNT_COMDEV							= $Party_Query_ComDevStatementData['PAYMENTAMOUNT'];
									$RECEIVEAMOUNT_COMDEV							= $Party_Query_ComDevStatementData['RECEIVEAMOUNT'];
									$PARTYNAME_COMDEV								= $Party_Query_ComDevStatementData['PARTYNAME'];
									
								}
									
									$Balance_Amount_ComDev			 = $SELLBILLAMOUNT_COMDEV + $PAYMENTAMOUNT_COMDEV  - $PURBILLAMOUNT_COMDEV - $RECEIVEAMOUNT_COMDEV ; 
									if($Balance_Amount_ComDev <= 0){
										$Rec_Amount_ComDev			= $RECEIVEAMOUNT_COMDEV + $PURBILLAMOUNT_COMDEV  - $SELLBILLAMOUNT_COMDEV - $PAYMENTAMOUNT_COMDEV;
									}else{
										$Pay_Amount_ComDev			= $SELLBILLAMOUNT_COMDEV + $PAYMENTAMOUNT_COMDEV  - $PURBILLAMOUNT_COMDEV - $RECEIVEAMOUNT_COMDEV ;
									}
								
								
								$Global_Rec_Amount_ComDev		 = $Global_Rec_Amount_ComDev + $Rec_Amount_ComDev ; 
								$Global_Pay_Amount_ComDev		 = $Global_Pay_Amount_ComDev + $Pay_Amount_ComDev ; 
							...
							$tableView .=" <tr>
											...
											<td align='right' valign='top' style='border: 1px dotted #000'>".number_format($Global_Rec_Amount_ComDev,2)."</td>
					
											<td align='right' valign='top' style='border: 1px dotted #000'>".number_format($Global_Pay_Amount_ComDev,2)."</td>
							...
							while($Loan_Payment_QueryStatementData		= mysql_fetch_array($Loan_Payment_QueryStatement)){	
								$LOANPAYMENT_ALU						= $Loan_Payment_QueryStatementData['LOANPAYMENTAMNT'];
								$INTERESTAMOUNT_ALU						= $Loan_Payment_QueryStatementData['INTAMNT'];
							...
							$Basta_Loan_Payment_Query 	= "SELECT sum(RECEIVEDAMOUNT) RECEIVEDAMOUNT,
																  sum(SELLPRICE) SELLPRICE
															FROM fna_basta
															WHERE ENTRYDATE BETWEEN '".$ENTRYDATE_FROM."' AND '".$ENTRYDATE_TO."' 
															
														";
							$Basta_Loan_Payment_QueryStatement					= mysql_query($Basta_Loan_Payment_Query);
							
							while($Basta_Loan_Payment_QueryStatementData		= mysql_fetch_array($Basta_Loan_Payment_QueryStatement)){	
								$BASTA_LOAN_RECEIVEDAMOUNT						= $Basta_Loan_Payment_QueryStatementData['RECEIVEDAMOUNT'];
								$BASTA_LOAN_SELLPRICE							= $Basta_Loan_Payment_QueryStatementData['SELLPRICE'];
							...
							$Income_Amount_Cold		= $RECEIVEAMOUNT_COLD + $Others_Income_Cold + $LOANPAYMENT_ALU + $BOOKINGMONEY_ALU + $SERVICECHARGE_FNA + $OTHERSINCOME_FNA + $INTERESTAMOUNT_ALU + $BASTA_LOAN_RECEIVEDAMOUNT ; 
							//$RECEIVEAMOUNT_COLD + $Others_Income_Cold + $LOANPAYMENT_ALU + $BOOKINGMONEY_ALU + $SERVICECHARGE_FNA + $OTHERSINCOME_FNA
							$Expanse_Amount_Cold	= $PAYMENTAMOUNT_COLD + $Expanse_Head_Cold	+ $PAYMENTAMOUNT_LAB_BILL + $LOANAMOUNT_ALU + $ADJUSTMENTMONEY_ALU + $TOTALCOMMISSION_ALU; 
					...
					$tableView .="
						...
						<tr style='font-weight:bold;'>
							...
							<td colspan='18' align='center' valign='top' style='border: 1px dotted #000'>Cash in Hand      : ".number_format($Global_Balance,2)."</td>
